...
def nrc_visualisation(dataframe, channelName):
    ...
    cm = ['r', 'coral', 'maroon', 'tomato', 'cyan',
          'chocolate', 'green', 'magenta', 'black', 'yellowgreen']
    ...
    for i, emotion in enumerate(emotions):
        ...
        ax[i].plot(y, linewidth=2, color=cm[i],
                   label=emotionName[0].capitalize())
        ax[i].set_xticks([])
        ax[i].legend()
        ax[i].set_ylim([0, 0.5])
...
def vader_afinn_vis(dataframe, constants, channelName):
    zero = 0
    dataframe['score_vader'] = dataframe['positive_vader']/100*1 + \
        dataframe['negative_vader']/100*-1+dataframe['neutral_vader']/100*0
    score_array_vader = dataframe['score_vader'].to_numpy()
    score_masked_vader = np.ma.masked_less_equal(score_array_vader, zero)
    dataframe['score_afinn'] = dataframe['positive_afinn']/100*1 + \
        dataframe['negative_afinn']/100*-1+dataframe['neutral_afinn']/100*0
...
def vader_pie(dataframe, channelName):
    ...
    line1 = plt.pie(lis1, labels=senti, startangle=90,
                    colors=my_colors, autopct='%.1f%%')
    ...
    senti = ["Positive", "Negative", "Neutral"]
    my_colors = ['green', 'red', 'orange']
    line2 = plt.pie(lis1, labels=senti, startangle=90,
                    colors=my_colors, autopct='%.1f%%')
...
def NRC_pie(dataframe, channelName):
    ...
    senti = ["positive", "negative", "Anger", "Anticipation",
             "Disgust", "Fear", "Joy", "Sad", "Surprise", "Trust"]
    my_colors = ['green', 'red', 'orange', 'silver', 'white',
                 'pink', 'purple', 'magenta', 'yellow', 'cyan']
    plt.pie(lis1, labels=senti, startangle=90,
            colors=my_colors, autopct='%.1f%%')
...
def fancySentiment(comments, channelName):
    ...
    sentiment = WordCloud(width=800, height=500,
                          random_state=21, max_font_size=110, max_words=100).generate(filtered_comments_str)
